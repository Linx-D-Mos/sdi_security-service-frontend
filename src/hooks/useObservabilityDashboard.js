...
export default function useObservabilityDashboard() {
  ...
  const fetchActiveRoutes = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost/api/v1/routes?paginate=true&include[]=routeStops.store&include[]=routeStops.routeStopState&filter[status]=in_progress&include[]=routeIncidents`, { headers });
